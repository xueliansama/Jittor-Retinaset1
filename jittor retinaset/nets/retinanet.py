...
class PyramidFeatures(nn.Module):
    ...
    def execute(self, inputs):
        ...
        P5_upsampled_x = jt.nn.interpolate(P5_x, size=(h4, w4), mode='bilinear')
        P4_x = P5_upsampled_x + P4_x
        P4_upsampled_x = jt.nn.interpolate(P4_x, size=(h3, w3), mode='bilinear')
...
class ClassificationModel(nn.Module):
    ...
    def execute(self, x):
        ...
        out2 = out1.reshape(batch_size, height, width, self.num_anchors, self.num_classes)
...
class retinanet(nn.Module):
    ...
    def _init_weights(self, fp16):
        ...
        prior = 0.01
        self.classificationModel.output.weight.fill_(0)
        if fp16:
            self.classificationModel.output.bias.fill_(-2.9)
        else:
            self.classificationModel.output.bias.fill_(-math.log((1.0 - prior) / prior))
        self.regressionModel.output.weight.fill_(0)
        self.regressionModel.output.bias.fill_(0)

    def execute(self, inputs):
        p3, p4, p5 = self.backbone_net(inputs)
        features = self.fpn([p3, p4, p5])

        regression = jt.contrib.concat([self.regressionModel(feature) for feature in features], dim=1)
        classification = jt.contrib.concat([self.classificationModel(feature) for feature in features], dim=1)
