...
class Anchors(nn.Module):
    ...
    def execute(self, features):  # Jittor中使用execute而不是forward
        hs = []
        ws = []
        # ----------------------------------------------------------#
        # 将输入进来的特征层的宽高记录进来
        ...
        for feature in features:
            _, _, h, w = feature.shape  # Jittor中使用shape而不是size()
            hs.append(h)
            ws.append(w)
        ...
        anchor_boxes = jt.array(anchor_boxes)  # 使用jt.array代替torch.from_numpy
        anchor_boxes = anchor_boxes.unsqueeze(0)  # Jittor的unsqueeze用法与PyTorch相同
